...
import {
  setSocietyBranding,
  getSocietyBranding,
  simpleChromeHeadHtml,
  simpleChromeFootHtml,
} from './branding.js';

export { setSocietyBranding } from './branding.js';
...
function currentParts(opts, chromeId) {
  ...
  if (id === 'simple' || !headerHtml) {
    headerHtml = headerHtml || simpleChromeHeadHtml();
    footerHtml = footerHtml || simpleChromeFootHtml();
  }
  ...
  if (showWm && !watermarkUrl && id !== 'simple' && id !== 'none') {
    watermarkUrl = getSocietyBranding().logoWatermark;
  }
  return {
    id,
    headerHtml,
