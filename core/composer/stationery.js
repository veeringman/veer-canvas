...
import { defaultStationeryFromBranding } from './branding.js';
...
const DEFAULT_LOGO = '/assets/favicon-192.png';
const DEFAULT_WM = '/assets/favicon-192.png';
...
export function defaultStationerySpec() {
  return defaultStationeryFromBranding();
}
...
function applyStationeryPreviewImages(host, spec) {
  const logoImg = host.querySelector('.mhws-st-logo');
  if (logoImg && spec.logo?.enabled && spec.logo?.src) {
    logoImg.src = spec.logo.src;
  }
  const wmImg = host.querySelector('img.wm');
  if (wmImg && spec.watermark?.enabled && spec.watermark?.src) {
    wmImg.src = spec.watermark.src;
    wmImg.style.opacity = String(spec.watermark.opacity ?? 0.72);
  }
}
...
export function paintStationeryPreview(host, specIn) {
  ...
  mount.innerHTML = sheet.html;
  const sheetNode = mount.firstElementChild;
  if (sheetNode) host.appendChild(sheetNode);
  applyStationeryPreviewImages(host, spec);
}
