"""Apple Wallet (.pkpass) signing for RWA vehicle parking passes.

Requires an Apple Developer Pass Type ID certificate on the server
(data/apple-wallet.env + Certificates.p12). Unsigned passes are rejected by iOS.
"""
...
def public_fields(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if not is_configured() or status not in ("active", "pending_renewal"):
    ...
    if code:
        url += f"?code={code}"
    return {"walletEnabled": True, "walletUrl": url}
