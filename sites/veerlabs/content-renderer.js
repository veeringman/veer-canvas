...
(function (global) {
  ...
  function detectFormat(body, explicitFormat) {
    ...
    if (/^graph\s+/m.test(trimmed) || /^flowchart\s+/m.test(trimmed) || /^sequenceDiagram/m.test(trimmed)) {
      return 'mermaid';
    }
    return 'markdown';
  }
  ...
  function renderMarkdown(md, context) {
    ...
    function flushCode() {
      ...
      if (codeLang === 'mermaid') {
        out += `<div class="mermaid">${code}</div>`;
      } else {
  ...
  async function initMermaid(root) {
    if (!root || typeof global.mermaid === 'undefined') return;
    ...
    try {
      await global.mermaid.run({ nodes: Array.from(nodes) });
    } catch (error) {
      nodes.forEach(node => {
        if (!node.dataset.mermaidError) {
          node.dataset.mermaidError = 'true';
          node.innerHTML = `<pre class="code-block mermaid-error">${escapeHtml(error.message || 'Mermaid render failed')}</pre>`;
        }
      });
    }
  }
